...
var Schema = require('@pulse/jugglingdb').Schema;
...
// transform raw db rows to the values for which our client is querying
function _rowsToValues(rows) {
...
proto._get = function(where) {
    ...
    var promise = new RSVP.Promise(function(resolve,reject) {
        self.promise.then(function(){
            self._AddonSettings.all({where:where}, function(err, arry){
                if (err) return reject(err);
                resolve(_rowsToValues(arry));
            });
        });
    });
    return promise;
...
// return a promise to a list of string tenant identifiers
...
proto.get = function (key, clientKey) {
  return this._get({key: key, clientKey: clientKey}).then(function (values) {
      return 0 == values.length ? null : values[0];
  });
};
