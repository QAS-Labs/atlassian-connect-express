...
function SequelizeAdapter(logger, opts) {
  ...
  if (opts.storage) {
    connectionOpts.storage = opts.storage;
  }

  var dbUrl = process.env["DATABASE_URL"] || opts.url;
  if (opts.type === 'sqlite') {
    dbUrl = dbUrl || "sqlite://"
  }

  var sequelize = new Sequelize(dbUrl, connectionOpts);
...
proto.isMemoryStore = function () {
  return this.settings.adapter === "sqlite" && this.settings.storage === ':memory:';
};
