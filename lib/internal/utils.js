...
utils.loadJSON = function loadConfig(path) {
    ...
    if (data) {
        var returnVal = {};
        try {
            returnVal = JSON.parse(uglify.minify('t=' + data, {fromString: true, output: {'quote_keys': true}, compress: {booleans: false}}).code.slice(2).replace(/;$/g, ''));
        } catch(e) {
            if (e instanceof uglify.JS_Parse_Error) {
                throw new Error('Failed to parse JSON file at (' + path + '). Error message: ' + e.message);
            } else {
                throw e;
            }
        }
