...
/*
 Bitbucket (de-)registration end-points:
    ...
    GET https://api.bitbucket.org/internal/account/USERNAME/addons

    POST https://api.bitbucket.org/internal/account/USERNAME/addons
        {url: "http://pow-location-3.herokuapp.com/descriptor.json"}

    DELETE https://api.bitbucket.org/internal/account/USERNAME/addons/ID
 */
...
function getRegisteredAddonId(addonKey) {
    return readCredentials().then(function (credentials) {
        return new RSVP.Promise(function (resolve, reject) {
            request.get({
                uri: 'https://api.bitbucket.org/internal/account/' + credentials.bitbucket.username + '/addons',
...
function deregisterAddon(addonId) {
    return readCredentials().then(function (credentials) {
        return new RSVP.Promise(function (resolve, reject) {
            request.del({
                uri: 'https://api.bitbucket.org/internal/account/' + credentials.bitbucket.username + '/addons/' + addonId,
                auth: asRequestAuth(credentials)
            }, function (error, response, body) {
                if (error) {
